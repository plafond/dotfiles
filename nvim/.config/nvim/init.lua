...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>ff', builtin.find_files, { desc = '[F]ind [F]iles alias' })
  ...
  --
  {
    ...
    config = function()
      -- Harpoon is a bookmarking plugin that allows you to quickly navigate to
      -- different files and directories. It's a bit like a file explorer, but
      -- it's more focused on quickly jumping to different places in your project.
      --
      -- You can add bookmarks with <leader>m and then jump to them with <C-hjkl>
      --
      -- You can also toggle the quick menu with <C-e> to see all of your bookmarks
      --
      -- For more information, see `:help harpoon`V
  ...
  -- Added this plugin.
  {
    ...
    config = function()
      ...
      local python = require('toggleterm.terminal').Terminal:new { cmd = 'python3', hidden = true }
